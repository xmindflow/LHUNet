...
def write_results(file, results, dataset):
...
def test(fold, dataset):
    path_validation = f"/hdd/LHUNET_NEW_CHECKPOINTS/V2/LUNG/lhunet_v22_r_t_rb_t_lr_0.003_GPU2 (copy)/Dataset709_Lung/lhunetLungTrainer__nnUNetPlans__3d_fullres/"
    
    if dataset == "lung":
        path_gt = "/hdd/nnunet_dataset/nnunet_raw/Dataset709_Lung/labelsTr/"
    elif dataset == "la":
        path_gt = "/hdd/nnunet_dataset/nnunet_raw/Dataset708_LAPreProcessed/labelsTr/"
    else:
        raise ValueError("Unsupported dataset. Choose from 'lung', 'la'.")
    path_gt="/hdd/nnunet_dataset/nnunet_raw/Dataset709_Lung/labelsTr/"
    
    label_list = sorted(glob.glob(os.path.join(path_gt, "*.nii.gz")))
    
    # infer_list = sorted(glob.glob(os.path.join(path_validation,f"fold_{fold}", "validation", "*.nii.gz")))
    infer_list = sorted(glob.glob(os.path.join(path_validation,f"crossval_results_folds_{fold}", "postprocessed", "*.nii.gz")))
    
    print("Loading success...")
    infer_base_names=[os.path.basename(infer_path) for infer_path in infer_list]
    label_list=sorted([label for label in label_list if os.path.basename(label) in infer_base_names])
    print('LEN LABELS',len(label_list))
    print('LEN PRED',len(infer_list))
    output_dir = os.path.join(path_validation, "inference_results", fold)
    ...
    write_results(output_file, results, dataset)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset", help="Dataset name for processing.", default="lung", choices=["lung", "la"])
    args = parser.parse_args()

    test(args.fold, args.dataset)
