...
my_output_identifier = "lhunet"
default_plans_identifier = "lhunet_Plansv2.1"
default_data_identifier = "lhunet_Data_plans_v2.1"
default_trainer = "lhunet_trainer_synapse"
...
base = (
    os.environ["lhunet_raw_data_base"]
    if "lhunet_raw_data_base" in os.environ.keys()
    else None
)
preprocessing_output_dir = (
    os.environ["lhunet_preprocessed"]
    if "lhunet_preprocessed" in os.environ.keys()
    else None
)
...
if base is not None:
    nnFormer_raw_data = join(base, "lhunet_raw_data")
    nnFormer_cropped_data = join(base, "lhunet_cropped_data")
    maybe_mkdir_p(nnFormer_raw_data)
    maybe_mkdir_p(nnFormer_cropped_data)
else:
    print(
        "lhunet_raw_data_base is not defined and model can only be used on data for which preprocessed files "
...
if preprocessing_output_dir is not None:
    ...
    print(
        "lhunet_preprocessed is not defined and model can not be used for preprocessing "
