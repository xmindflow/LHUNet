...
def restore_model(pkl_file, checkpoint=None, train=False, fp16=None, folder=None):
    ...
    tr = recursive_find_python_class(
        [search_in], name, current_module="unetr_pp.training.network_training"
    )
    ...
    if tr is None:
        raise RuntimeError(
            "Could not find the model trainer specified in checkpoint in unetr_pp.trainig.network_training. If it "
