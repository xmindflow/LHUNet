...
class DeformConvPack_experimental(DeformConv):
    def __init__(
        ...
        super(DeformConvPack_experimental, self).__init__(
            in_channels,
            out_channels,
            kernel_size,
            stride,
        ...
        self.channel_adjust = nn.Conv3d(
            self.in_channels, self.out_channels, kernel_size=(1, 1, 1)
        )
        self.conv_offset = nn.Conv3d(
            self.out_channels,
            self.out_channels,
...
if __name__ == "__main__":
    deform_Conv_3D = DeformConvPack(
        32, 32, kernel_size=(7, 7, 7), padding=9, dilation=3
    ).cuda(0)
    deform_Conv_3D_exp = DeformConvPack_experimental(
        32, 32, kernel_size=(7, 7, 7), padding=9, dilation=3
    ).cuda(0)
    ...
    n_parameters = sum(
        p.numel() for p in deform_Conv_3D.parameters() if p.requires_grad
    )
    n_parameters_exp = sum(
        p.numel() for p in deform_Conv_3D_exp.parameters() if p.requires_grad
    )
