...
class nnUNetPredictor(object):
    ...
    def initialize_from_trained_model_folder(self, model_training_output_dir: str,
        ...
        ######### YOUSEF HERE!!! ##############
        ########## A Dirty Hack to make sure the batch and patch size are set correctly for nnUNetTrainer ##########
        ############ It should be fixed in the future ############
        if self.configuration_manager.configuration["patch_size"] == [96, 128, 192]:
            print("We are using LA trainer, so we set the patch size to [96, 96, 96]")
            self.configuration_manager.configuration["patch_size"] = [96, 96, 96]
        elif self.configuration_manager.configuration["patch_size"] == [80, 192, 160]:
            print("We are using Lung trainer, so we set the patch size to [32, 192, 192]")
            self.configuration_manager.configuration["patch_size"] = [32, 192, 192]
        elif self.configuration_manager.configuration["patch_size"] == [128, 128, 128]:
            print("We are using Brats trainer, so we set the patch size to [128, 128, 128]")
        elif self.configuration_manager.configuration["patch_size"] == [96, 160, 160]:
            print("We are using Synapse trainer, so we set the patch size to [64, 128, 128]")
    ...
    def predict_from_files(self,
        ...
        return self.predict_from_data_iterator(data_iterator, save_probabilities, num_processes_segmentation_export, amp=amp)
    ...
    def predict_from_data_iterator(self,
        ...
        with multiprocessing.get_context("spawn").Pool(num_processes_segmentation_export) as export_pool:
            ...
            for preprocessed in data_iterator:
                ...
                prediction = self.predict_logits_from_preprocessed_data(data, amp=amp).cpu().detach().numpy()
    ...
    def predict_single_npy_array(self, input_image: np.ndarray, image_properties: dict,
        ...
        if self.verbose:
            print('predicting')
        predicted_logits = self.predict_logits_from_preprocessed_data(dct['data'], amp=amp).cpu()
    ...
    def predict_logits_from_preprocessed_data(self, data: torch.Tensor, amp: bool = True) -> torch.Tensor:
        """
        ...
        for params in self.list_of_parameters:
            ...
            if prediction is None:
                prediction = self.predict_sliding_window_return_logits(data, amp=amp).to('cpu')
            else:
                prediction += self.predict_sliding_window_return_logits(data, amp=amp).to('cpu')
    ...
    @torch.inference_mode()
    def predict_sliding_window_return_logits(self, input_image: torch.Tensor, amp:bool = True) \
        ...
        with torch.autocast(self.device.type, enabled=True) if (self.device.type == "cuda" and amp) else dummy_context():
    ...
    def predict_from_files_sequential(self,
        ...
        for li, of, sps in zip(list_of_lists_or_source_folder, output_filename_truncated, seg_from_prev_stage_files):
            ...
            prediction = self.predict_logits_from_preprocessed_data(torch.from_numpy(data), amp=amp).cpu()
...
def predict_entry_point_modelfolder():
    ...
    parser.add_argument('--disable_progress_bar', action='store_true', required=False, default=False,
                        help='Set this flag to disable progress bar. Recommended for HPC environments (non interactive '
                             'jobs)')
    parser.add_argument('--amp', action='store_true', required=False, default=True,
                        help='Set this flag to enable automatic mixed precision inference. This will speed up inference '
                             'on GPUs. Default: False.')
    ...
    predictor.predict_from_files(args.i, args.o, save_probabilities=args.save_probabilities,
                                 ...
                                 num_parts=1, part_id=0, amp=args.amp)
...
def predict_entry_point():
    ...
    if run_sequential:
        ...
        predictor.predict_from_files_sequential(args.i, args.o, save_probabilities=args.save_probabilities,
                                                overwrite=not args.continue_prediction,
                                                folder_with_segs_from_prev_stage=args.prev_stage_predictions, amp=args.amp)
        ...
        predictor.predict_from_files(args.i, args.o, save_probabilities=args.save_probabilities,
                                    ...
                                    part_id=args.part_id, amp=args.amp)
